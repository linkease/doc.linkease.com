...
DDNSTO 在保护用户数据安全同时也要避免恶意分享不良内容带来的法律风险，所以我们对用户的域名访问都做了认证判断：服务器会判断用户当前IP是否已经过验证，如果没有就会跳转验证流程。

所以当你的第三方客户端使用 DDNSTO 时，需要先访问一遍对应域名进行身份验证。否则客户端可能会出现连接失败或假死等错误。
...
![访问成功](../resources/Authentication/wx01.png)

![访问成功](../resources/Authentication/wx02.png)
...
2、选择“添加当前 IP”或者添加其他网络 IP，最多 8个：
...
1、[DDNSTO 控制台](https://www.ddnsto.com/app/#/login)——设备管理——设备，在需要标记域名的右侧，点击对应域名的「标记客户端」按钮：

![访问成功](../resources/Authentication/tag01.png)

弹窗继续点击「开始标记」：

![访问成功](../resources/Authentication/tag02.png)

2、提示"正在标记"，在两分钟内，复制域名使用客户端（浏览器等）打开，开始标记：

![访问成功](../resources/Authentication/tag03.png)

3、界面显示「已标记客户端」即成功，此域名即可在当前客户端（48小时内）上免验证 IP 直接访问。

![访问成功](../resources/Authentication/tag04.png)
